...
Class ATForegroundColor24 : ConsoleColor24 {
    ATForegroundColor24(): base() {}

    [String]ToAnsiControlSequenceString() {
        Return [ATControlSequences]::GenerateFG24String($this)
    }
}

Class ATBackgroundColor24 : ConsoleColor24 {
    ATBackgroundColor24(): base() {}

    [String]ToAnsiControlSequenceString() {
        Return [ATControlSequences]::GenerateBG24String($this)
    }
}

Class ATForegroundColor24None : ATForegroundColor24 {
    ATForegroundColor24None(): base() {}
...
Class WindowBase {
    ...
    [String]$WindowBorderHorizontal
    [String]$WindowBorderVertical
    [Int]$Width
    [Int]$Height
    [Int]$WindowLTRow
    [Int]$WindowLTColumn
    ...
    WindowBase() {
        ...
        $this.Height = 0
        $this.WindowLTRow = 0
        $this.WindowLTColumn = 0
        $this.WindowRBRow = 0
        $this.WindowRBColumn = 0
    }

    [Void]Initialize() {
        $this.LeftTop          = [ATCoordinates]::new($this.WindowLTRow, $this.WindowLTColumn)
        $this.RightBottom      = [ATCoordinates]::new($this.WindowRBRow, $this.WindowRBColumn)
        $this.BorderDrawColors = @(
            [CCWhite24]::new(),
            [CCWhite24]::new(),
            [CCWhite24]::new(),
                ...
                UserData = $(
                    Invoke-Command -ScriptBlock {
                        ...
                        For($a = 0; $a -LT $this.Height; $a++) {
                            [ATCoordinates]$coords = [ATCoordinates]@{
                                Row    = ($this.LeftTop.Row + 1) + $a
                                Column = $this.LeftTop.Column
                            }
                            $temp += "$($this.BorderStrings[[WindowBase]::BorderStringVertical])$($coords.ToAnsiControlSequenceString())"
                        }
    ...
    [Void]UpdateDimensions() {
        $this.Width   = $this.RightBottom.Colum - $this.LeftTop.Column
